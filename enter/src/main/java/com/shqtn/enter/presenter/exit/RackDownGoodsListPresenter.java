...
public class RackDownGoodsListPresenter extends AbstractListActivityPresenter {

    private String mOperateRack;
    private String mOperateManifest;
    private CommonAdapter<RackDownGoods> mGoodsListAdapter;
    ...
    private ResultCallback mGoodsListCallback = new ResultCallback() {
        ...
        public void onSuccess(ResultBean response) {
            mGoodsList = getRows(response.getData(), RackDownGoods.class);
     ...
     */
    private void toRackDownGoodsOperateActivity(RackDownGoods rackDownGoods, double initQty) {
    ...
    public void decodeGoods(CodeGoods goods) {
        ...
        ArrayList<RackDownGoods> arraylist = GoodsUtils.getManifestOfGoodsSame(mGoodsList, goods);
        getView().cancelProgressDialog();
        if (arraylist == null) {
             ...
             * 选择数量最大的进行操作
             */
            RackDownGoods maxQtyRackDownGoods = null;
            for (int i = 0; i < arraylist.size(); i++) {
                RackDownGoods item = arraylist.get(i);
                if (i == 0) {
    ...
    private boolean isSubmitLpn(CodeLpn lpn) {
        ...
        GoodsUtils.C<RackDownGoods, CodeGoods> b = GoodsUtils.isQuantitySatisfactory(lpn.getGoodsList(), mGoodsList);
        //判断数量是否满足
        if (!b.isSatisfactory) {
            double planQty = 0;
