...
public class YTDepotOutManifestPresenter extends AbstractListActivityPresenter {
    private ArrayList<DepotOutManifest> mManifestList = new ArrayList<>();

    private ArrayList<DepotOutManifest> mSearchManifestList;
    ...
    public void init() {
        ...
        mManifestAdapter = new CommonAdapter<DepotOutManifest>(getAty().getContext(), null, com.shqtn.enter.R.layout.item_manifest) {
            @Override
            public void setItemContent(ViewHolder holder, DepotOutManifest depotOutManifest, int position) {
                holder.setLabelText(com.shqtn.enter.R.id.item_manifest_ltv, depotOutManifest.getDeliveryNo());
            }
        };
        ...
        view.setEditTextHint("请输入任务单号，货品");
        ...
        mManifestListParams.setWhCode(whcode);
    ...
    public void decodeManifest(CodeManifest manifest) {
        ...
        if (mSearchManifestList == null) {
            getView().displayMsgDialog(String.format("当前列表中不存在任务单号:%s", manifest.getDocNo()));
            return;
        }
        ...
        for (DepotOutManifest depotOutManifest : mSearchManifestList) {
    ...
    public void clickItem(int position) {
        DepotOutManifest depotOutManifest = mSearchManifestList.get(position - 1);
        toGoodsListActivity(depotOutManifest);
    }
